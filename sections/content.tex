...
These methods can also be sensitive when handling inexact representations. 
\\
...
In the following, the considered boolean set-operations are intersection, union and subtraction.
...
For the latter, tree depths have increased by $41.7\%$ (model M$1$) to $182.5\%$ (model M$2$) compared to the input tree, while for baseline approaches, an increase of $0.0\%$ (model M$1$) to $130.0\%$ (model M$2$) is visible.
Tree sizes show similar behavior:
Partitioning variants produce $44.4\%$ (model M$2$) to $82.2\%$ (model M$0$) larger trees, while baseline approaches increase tree size by only $18.6\%$ (model M$2$) to $25.0\%$ (model M$1$).
Comparing tree sizes between partitioning and baseline approaches directly reveals that the former results in $21.7\%$ (model M$2$) to $181.8\%$ (model M$3$) larger trees.
\\
...
\begin{figure*}
	...
	\end{subfigure}	
	\vskip\baselineskip	
	\caption{Results of all pipeline steps for model M$1$.}
	\label{fig:models}
\end{figure*}
\begin{figure*}
	\centering
