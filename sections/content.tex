...
The first term in Equation \ref{eq:of} (under the sum) estimates how close the surface induced by $t$ matches the point cloud, while the second term penalizes large (in terms of number of nodes) trees.
...
Both options were implemented for multi-core processors and evaluated in Section~\ref{eq:of}.
...
The merge process has an asymptotic computational complexity of $\mathcal{O}(\vert L \vert^2)$ since in worst case $L$ has to be completely traversed for each merge.
...
It outperforms baseline by a factor of $15.3$ (single-threaded, BST) and $7.5$ (multi-threaded, BMTGA) on average.
...
For model $3$ SMTGA is again the fastest method. 
...
Since the focus is on performance, this is acceptable behavior.

\paragraph{Scaling with respect to point cloud size}  
...
%%%%%%%%%%%%%%%%%%
\section{Conclusion}
We propose in this work a technique for accelerating an evolutionary algorithm for extracting 
a \ac{CSG} tree from a point cloud. Our method is based on a partitioning of the search space obtained 
...
Our experimental evaluation indicated a significant speed-up over the baseline approach (the evolutionary algorithm). 
\\
