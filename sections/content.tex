...
In this section, some related works in these domains are briefly discussed. 
...
\\
For better scalability, computational complexity can be reduced from $\mathcal{O}({n_p}^2)$ (overlap check between each primitive and each other primitive) to $\mathcal{O}(n_p\log(n_p))$ using e.g. Octrees \cite{meagher1982Octree}.
...
\begin{figure}[htb]
	...
	\end{subfigure}
	\vskip\baselineskip	
	\caption{In (b), B influences the per-partition solution without being part of the partition.}\label{fig:part}
\end{figure}
...
A simple union of all tree root nodes leads to incorrect results if primitives that are part of multiple cliques are not splitted.
...
\begin{enumerate}
	...
	Extracted trees might contain artefacts that affect their mergeability (e.g. intersections with the same primitive as operands). 
	Foreach tree in $L$, these parts are transformed by traversing the tree and replacing the aforementioned patterns iteratively with their simplifications (e.g. replacing the union of the same primitive with the primitive itself). 
	The process is repeated until no more parts can be transformed that way.  
...
\begin{figure}[htb]
	...
	\caption{Timings for all approach combinations and models $0$ and $2$ with high-detail sampling (black lines: standard deviations).}
	\label{fig:graph1}
\end{figure}
\begin{figure}[htb]
	\centering
	\includegraphics[width=0.5\textwidth]{figures/g4.pdf}
	\caption{Timings for all approach combinations and models $1$ and $3$ with high-detail sampling (black lines: standard deviations).}
	\label{fig:graph4}
\end{figure}
\begin{figure}[htb]
	\centering
	\includegraphics[width=0.5\textwidth]{figures/g3.pdf}
	\caption{Ratio between high-detail and low-detail point cloud size factor and corresponding timing factors for all models (see Equation \ref{eq:ratio}). The red line indicates linear scaling with a slope of $1$ with respect to point cloud size. Model $3$ exists only in high-detail.}
	\label{fig:graph3}
\end{figure}
\begin{figure}[htb]
	\centering
